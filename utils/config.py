...
from aidd_codebase.utils.directories import validate_or_create_file
...
class DataclassHandler(_ABCHandler):
    ...
    def as_config_data(obj: Any) -> Dict[str, Tuple[Any, type]]:
        return {
            f.name: (f.default, f.type)
            if not isinstance(f.default, _MISSING_TYPE)
            else (f.default_factory(), f.type)
            for f in fields(obj)
        }

    @staticmethod
    def set_dataclass(
        dataclass_obj: Any, arguments: Dict[str, Any]
    ) -> _ABCDataClass:
        try:
            dataclass_instance = dataclass_obj(**arguments)
        except Exception:
            raise Exception(
                "Dataclass initiation went wrong."
                + "Most likely missing field values in config."
            )
        return dataclass_instance

    @staticmethod
    def get_dataclass(call_name: str, obj: Dict[str, Any]) -> _ABCDataClass:
        ...
        dataclass_fields = [
            choose_field(key, field_value) for key, field_value in obj.items()
        ]
...
class Config:
    ...
    config_data: Dict[str, Dict[str, Tuple[Any, type]]] = field(
        default_factory=lambda: {}
    )
    data_classes: Dict[str, _ABCDataClass] = field(default_factory=lambda: {})

    def to_environment(self, key: str, value: Any):
        self.__dict__[key] = value[0]

    def to_config_data(
        self, group_name: str, obj: Any, handler: _ABCHandler
    ) -> None:
        self.config_data[group_name] = handler.as_config_data(obj)

    def overwrite_config_data(
        self, group_name: str, obj: Any, handler: _ABCHandler
    ) -> None:
    ...
    def processing_arguments(
        self, group: str, obj: Any, handler: _ABCHandler
    ) -> None:
        obj_data = handler.as_config_data(obj)
    ...
    def return_dataclass(self, call_name: str) -> _ABCDataClass:
        """Returns current configuration of a call_name as a dataclass."""
        if call_name in self.data_classes.keys():
            return DataclassHandler.set_dataclass(
                self.data_classes[call_name], self.return_arguments(call_name)
            )
        else:
            return DataclassHandler.get_dataclass(
                call_name, self.return_arguments(call_name)
            )
    ...
    def generate_yaml(
        self, file_location: str, call_name: Optional[str] = None
    ) -> None:
        """Generates a YAML file with the current configuration."""
        if call_name:
            YamlHandler.dict_to_yaml(
                file_location, self.return_arguments(call_name)
            )
        else:
            self_dict = {
                k: v
                for k, v in self.return_self().items()
                if k != "config_data"
            }
